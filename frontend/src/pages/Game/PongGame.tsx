...
const PongGame: React.FC = () => {
    ...
    useEffect(() => {
        ...
        socket?.on(
            'createLobby',
            (lobbyId: string, userName1: string, userName2: string) => {
                setLobbyId(lobbyId);
                ...
                setTimeout(() => {
                    setShowGo(true), setCountdown(null);
                }, 3000);
                setTimeout(() => {
    ...
    const handleGameEnd = (data: any, forfait: boolean) => {
        ...
        setShowBall(false);
        if (data === socket?.id) {
            if (forfait) setGameEndMessage('You win by forfait!');
            else setGameEndMessage('You win!');
            console.log('LobbyId: ', lobbyId);
            setTimeout(() => {
                socket?.emit('destroyLobby', { lobbyId });
            }, 1000);
        } else {
