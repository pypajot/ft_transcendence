...
export const BlockUser = ({ open, setOpen, setFriendError }: any) => {
    ...
    const { setRenderConversation } = useChatContext();
    ...
    const handleClickOutside = (event: MouseEvent) => {
        if (
            ...
            if (open === 'blockUser') setOpen(null);
        }
    };
    ...
    const BlockUser = useCallback(
        ...
        [user, socket, setRenderConversation, setFriendError, setSocketError]
    );
    ...
    return (
        <div className="block-user-dropdown" ref={dropdownRef}>
            <button className="block-user-button" onClick={handleOpen}>
                Block users
            </button>
            {open === 'blockUser' ? <BlockUserDropdown /> : null}
        </div>
    );
};
